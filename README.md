...
**v1.2.5** — MV3 browser extension that swaps the YouTube watch player for an **ad-free in-page player**, with MSE adaptive seek (auto re-resolve on CDN 403), theater/wide mode, chapters, YouTube-style hotkeys (incl. **t**), remembered quality (Settings ⚙ submenu), Always Ad-Free (`/watch` + `/live`), session diagnostic export, captions, background playback, and optional download tooling.
